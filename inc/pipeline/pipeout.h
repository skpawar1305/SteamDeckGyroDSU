...
template <class T> class PipeOut {
public:
  ...
  T &GetDataToFill();
  // Get reference to unique_ptr to object to fill. Needs to be obtained only
  // once.
  ...
  T &GetData();
  // Get reference to unique_ptr to received data. Wait for data using
  // WaitForData().
