...
Thread::Thread() : executeThread(), stopMutex(), stop(false) {}
...
void Thread::TryStopThenKill() { TryStopThenKill(cTimeout); }

void Thread::TryRestartThenForceRestart() {
  TryRestartThenForceRestart(cTimeout);
}

bool Thread::IsStarted() { return executeThread != nullptr; }
