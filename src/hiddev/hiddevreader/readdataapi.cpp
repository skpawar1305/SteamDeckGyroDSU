...
HidDevReader::ReadDataApi::ReadDataApi(uint16_t const &_vId,
                                       uint16_t const &_pId,
                                       const int &_interfaceNumber,
                                       int const &_frameLen,
                                       int const &_scanTimeUs)
    : vId(_vId), pId(_pId), ReadData(_frameLen),
      timeout(cApiScanTimeToTimeout * _scanTimeUs / 1000),
      interfaceNumber(_interfaceNumber), noGyro(nullptr) {}
...
void HidDevReader::ReadDataApi::Execute() {
  ...
  if (!dev.Open())
    throw std::runtime_error(
        "HidDevReader::ReadDataApi: Problem opening HID device.");
  ...
  while (ShouldContinue()) {
    ...
    if (readCnt < data->size()) {
      {
        LogF(LogLevelTrace)
            << "HidDevReader::ReadDataApi: Not enough bytes read: " << readCnt
            << ".";
      }
      continue;
    }
    ...
    if (readCnt == 0) {
      Log("HidDevReader::ReadDataApi: Waiting for data timed out.",
          LogLevelTrace);
      continue;
    }
