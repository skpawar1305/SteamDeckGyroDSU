...
HidDevReader::ProcessData::ProcessData(int const &_frameLen, ReadData &_data,
    ...
    : readData(_data), data(_data.Data), ReadStuck(),
      timeout(cApiScanTimeToTimeout * scanTimeUs),
      Frame(new frame_t(_frameLen), new frame_t(_frameLen),
            new frame_t(_frameLen)) {}

HidDevReader::ProcessData::~ProcessData() { TryStopThenKill(); }
...
void HidDevReader::ProcessData::Execute() {
  ...
  while (ShouldContinue()) {
    if (!data.WaitForData(timeout)) {
      Log("HidDevReader::ProcessData: Reading from hiddev file stuck. "
          "Force-restarting reading task.",
          LogLevelDebug);
    ...
    for (int i = 0, j = cByteposInput; i < frame->size();
         ++i, j += cInputRecordLen) {
      (*frame)[i] = (*hidData)[j];
    }

    HandleMissedTicks("HidDevReader::ProcessData", "frames",
                      Frame.WasReceived(), missedTicks,
                      cReportMissedTicksPeriod, nonMissedLossTicks);
...
void HidDevReader::ProcessData::FlushPipes() { data.SendData(); }
} // namespace kmicki::hiddev