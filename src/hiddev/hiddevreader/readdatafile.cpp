...
HidDevReader::ReadDataFile::ReadDataFile(std::string const &_inputFilePath,
                                         int const &_frameLen,
                                         int const &_scanTimeUs)
    : inputFile(_inputFilePath, cFileScanTimeToTimeout * _scanTimeUs, false),
      ReadData(_frameLen * HidDevReader::cInputRecordLen) {}
...
void HidDevReader::ReadDataFile::Execute() {
  ...
  if (!inputFile.IsOpen()) {
    ...
    throw std::runtime_error("HidDevReader::ReadDataFile: Problem opening "
                             "hiddev file. Are priviliges granted?");
  }
  ...
  while (ShouldContinue()) {
    ...
    if (readCnt == 0) {
      Log("HidDevReader::ReadDataFile: Waiting for data timed out.",
          LogLevelTrace);
      continue;
    }
    ...
    HandleMissedTicks("HidDevReader::ReadData", "HID frames",
                      Data.WasReceived(), missedTicks, cReportMissedTicksPeriod,
                      nonMissedTicks);
...
bool HidDevReader::ReadDataFile::CheckData(
    std::unique_ptr<std::vector<char>> const &data, ssize_t readCnt) {
  ...
  if (!inputFail) {
    ...
    if (startMarkerFail && startMarker.size() > 0 &&
        ExtractFirst4Bytes(*data) == cFirst4BytesAlternative) {
      startMarkerFail = false;
      // Check special start marker
      for (int i = cByteposInput, j = 0; j < startMarker.size();
           ++j, i += cInputRecordLen)
  ...
  if (inputFail || startMarkerFail) {
    if (inputFail)
      Log("HidDevReader::ReadDataFile: Reading from hiddev file failed.",
          LogLevelDebug);
    else
      Log("HidDevReader::ReadDataFile: Reading from hiddev file started in the "
          "middle of the HID frame.",
          LogLevelDebug);
