...
HidDevReader::ServeFrame::~ServeFrame() { TryStopThenKill(); }
...
void HidDevReader::ServeFrame::HandleMissedFrames(
    int &serveCnt, std::vector<int> &missedTicks,
    std::vector<int> &nonMissedTicks, std::vector<std::string> &serveNames) {
  ...
  for (int i = 0; i < frames.size(); ++i)
    HandleMissedTicks(serveNames[i], "frames", frames[i]->WasConsumedNoLock(),
                      missedTicks[i], cReportMissedTicksPeriod,
                      nonMissedTicks[i]);
}
...
std::vector<Serve<HidDevReader::frame_t>::ServeLock>
HidDevReader::ServeFrame::GetServeLocks() {
