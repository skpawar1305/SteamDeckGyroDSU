...
// Constants
const int HidDevReader::cInputRecordLen =
    8; // Number of bytes that are read from hiddev file per 1 byte of HID data.
const int HidDevReader::cByteposInput =
    4; // Position in the raw hiddev record (of INPUT_RECORD_LEN length) where
       // HID data byte is.
...
void HandleMissedTicks(std::string name, std::string tickName, bool received,
  ...
  if (!received) {
    ...
    if (ticks % period == 0) {
      {
        LogF(LogLevelDebug)
            << name << ": Missed " << period << " " << tickName << " after "
            << nonMissed << " " << tickName << ". Still being missed.";
      }
      if (ticks > period)
        ticks -= period;
    }
    ...
    {
      LogF(LogLevelDebug) << name << ": Missed " << ((ticks + 1) % period - 1)
                          << " " << tickName << ". Not being missed anymore.";
    }
    ticks = 0;
    nonMissed = 0;
  } else if (ticks > 0) {
    {
      LogF(LogLevelDebug) << name << ": Missed " << ticks << " " << tickName
                          << " after " << nonMissed << " " << tickName << ".";
    }
    ticks = 0;
    nonMissed = 0;
  } else
...
void HidDevReader::ConstructPipeline(ReadData *_readData, int const &_frameLen,
...
HidDevReader::HidDevReader(int const &hidNo, int const &_frameLen,
                           int const &scanTimeUs)
    : frameLen(_frameLen), startStopMutex(), readDataApi(nullptr) {
  if (hidNo < 0)
    throw std::invalid_argument("hidNo");
...
HidDevReader::HidDevReader(uint16_t const &vId, uint16_t const &pId,
                           int const &interfaceNumber, int const &_frameLen,
                           int const &scanTimeUs)
    : frameLen(_frameLen), startStopMutex() {
  readDataApi =
      new ReadDataApi(vId, pId, interfaceNumber, _frameLen, scanTimeUs);

  ConstructPipeline(readDataApi, _frameLen, scanTimeUs, false);
}

HidDevReader::~HidDevReader() { Stop(); }
...
void HidDevReader::Start() {
  std::lock_guard startLock(
      startStopMutex); // prevent starting and stopping at the same time
