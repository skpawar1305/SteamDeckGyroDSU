...
inline std::string GetHexStringId(uint16_t id) {
  return std::move(((std::ostringstream())
                    << std::setfill('0') << std::setw(4) << std::nouppercase
                    << std::hex << id)
                       .str());
}
...
int FindHidDevNo(uint16_t vid, uint16_t pid) {
  ...
  //   for (const auto &hiddevFile :
  //        std::filesystem::directory_iterator(cHiddevPath)) {
  //     // Get hiddev* device
  //     sd_device *hidDevice = NULL;
  //     if (sd_device_new_from_devname(&hidDevice, hiddevFile.path().c_str())
  //     != 0)
  //       continue;

  //     // Go up to usb_device
  //     sd_device *usbDevice = NULL;
  //     if (sd_device_get_parent_with_subsystem_devtype(
  //             hidDevice, cSubsystem.c_str(), cDevType.c_str(), &usbDevice) !=
  //             0)
  //       continue;

  //     // Check vid,pid
  //     const char *product = NULL;
  //     if (sd_device_get_property_value(usbDevice,
  //     cProductPropertyName.c_str(),
  //                                      &product) != 0)
  //       continue;
  //     if (vidStr == std::string(product).substr(0, 4) &&
  //         pidStr == std::string(product).substr(5, 4)) {
